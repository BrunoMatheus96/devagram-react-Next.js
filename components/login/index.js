...
export default function Login({aposAutenticacao}) {
  const [email, setEmail] = useState(""); //Vai salvar a o email
  const [senha, setSenha] = useState(""); //Vai salvar a senha
      ...
      <div className="conteudoPaginaPublica">
        <form onSubmit={aoSubmeter}>
          <InputPublico
            ...
            tipo="email"
            //Esse (e) é de evento
            aoAlterarValor={(e) => setEmail(e.target.value)} //e.target é a referencia do objeto JS correspondente ao input 
            valor={email}
        ...
        <div className="rodapePaginaPublica">
          ...
          <Link href={"/cadastro"}>Faça seu cadastro agora!</Link> {/*Link faz o redirecionamento de uma páguina para outro*/}
        </div>
      </div>
    </section>
  );
